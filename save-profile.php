...
if ( true ) {
  ...
  try
  {
    $db_conn = new PDO( "mysql:hostname=localhost;dbname=Usuario;charset=utf8", "nick", "admin" );
    ...
    if( !empty($data['nombre']) ) {
      ...
      //  En caso de que el id sea cualquier otra cosa NO mayor que 0 (linea 30)
        ...
        $query = 'INSERT INTO Usuario
            (nombre, apellido, :email, password, img, edad)
          VALUES
            (:nombre, :apellido, :email, :password, :img, :edad)';
        ...
        if ( $q->execute($params) ) {
          ...
          $q = $db_conn->prepare("SELECT id FROM usuarios ORDER BY id DESC LIMIT 1");
          $q->execute();
